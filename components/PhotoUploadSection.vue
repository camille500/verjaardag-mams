<template>
  <section class="memories-section" id="memories" ref="sectionRef">
    <div class="container">
      <!-- Header -->
      <div class="header" :class="{ visible: isVisible }">
        <span class="section-label">{{ $t('memories.title') }}</span>
        <p class="section-subtitle">{{ $t('memories.subtitle') }}</p>
      </div>
      ...
      <Transition name="success-reveal">
        <div v-if="currentStep === 'success'" class="success-container">
          ...
          <button @click="resetAll" class="secondary-btn">
      ...
      <Transition name="step-fade" mode="out-in">
        <!-- Step 1: Welcome / Name -->
        <div v-if="currentStep === 'intro'" :key="'intro'" class="step-content initial" :class="{ visible: isVisible }">
          <p class="intro-text">{{ $t('memories.intro') }}</p>

          <div class="name-input-section">
            <label for="memory-name" class="field-label">
              {{ $t('memories.nameLabel') }}
            </label>
            <input
              id="memory-name"
              type="text"
              ...
              class="text-input"
              @keyup.enter="goToTypeSelection"
            />
            <p class="field-helper">{{ $t('memories.nameHelper') }}</p>
          </div>

          <button
            @click="goToTypeSelection"
            class="primary-btn"
            :disabled="!userName.trim()"
          >
            {{ $t('memories.startButton') }}
        ...
        <!-- Step 2: Choose Type -->
        <div v-else-if="currentStep === 'type'" :key="'type'" class="step-content">
          <button @click="currentStep = 'intro'" class="back-link">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
type Step = 'intro' | 'type' | 'photos' | 'text' | 'success'
...
const submitMemory = async () => {
  if (!userName.value.trim()) {
    errorMessage.value = t('memories.errorName')
    return
  }
  ...
  try {
    ...
    formData.append('uploaderName', userName.value.trim())
...
/* Initial animation for first step only */
